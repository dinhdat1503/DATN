...
def print_comparison_table(results_list: list[dict]) -> str:
    """In bảng so sánh các thực nghiệm và trả về nội dung dạng markdown."""
    if not results_list:
    ...
    # Tự động phát hiện xem có thực nghiệm phân loại nhị phân (Phase 1) không
    # (Nếu không có F1 score của các nhãn bệnh lý cụ thể thì là nhị phân)
    is_binary = any(r.get("test_f1_D") is None for r in results_list)

    if is_binary:
        # --- Bộ metrics nhị phân y sinh ---
        main_metrics = [
            ("test_f1_macro",       "F1-Score",         ".4f"),
            ("test_auc_roc",        "AUC-ROC",          ".4f"),
            ("test_accuracy",       "Accuracy",         ".4f"),
            ("test_sensitivity",    "Sensitivity (Độ nhạy)", ".4f"),
            ("test_specificity",    "Specificity (Độ đặc hiệu)", ".4f"),
            ("test_age_mae",        "Age MAE (years)",  ".2f"),
            ("test_age_pearson",    "Age Pearson r",    ".4f"),
        ]
        per_label_metrics = []
    else:
        # --- Bộ metrics đa nhãn gốc ---
        main_metrics = [
    ...
    lines = []
    lines.append("\n" + "="*72)
    lines.append("  KẾT QUẢ SO SÁNH THỰC NGHIỆM ĐỒ ÁN TỐT NGHIỆP ODIR-5K")
    lines.append("="*72)

    # In tên cột thực nghiệm
    col_width = max(len(n) for n in exp_names) + 2
    header = f"  {'Metric':<35}" + "".join(f"{n:>{col_width}}" for n in exp_names)
    lines.append(header)
    lines.append("  " + "─"*70)

    # In metrics phân loại chính
    lines.append("\n  [A] CLASSIFICATION METRICS")
    num_cls_metrics = 5 if is_binary else 5
    for key, label, fmt in main_metrics[:num_cls_metrics]:
        row = f"  {label:<35}"
    ...
    # In metrics hồi quy tuổi võng mạc
    ...
    # In F1 score của từng bệnh (chỉ khi ở chế độ đa nhãn Phase 2)
    ...
    # --- Phân tích cải thiện Delta (nếu so sánh đúng 2 thực nghiệm) ---
    if len(results_list) == 2:
        r_raw, r_enh = results_list[0], results_list[1]
        lines.append("\n  [D] PHÂN TÍCH BIẾN ĐỘNG DELTA (Thực nghiệm sau - Thực nghiệm trước)")
        lines.append("  " + "─"*70)
        ...
        for key, label, fmt in all_metrics:
            ...
            delta = v_enh - v_raw
            # Với MAE tuổi, delta âm nghĩa là cải thiện (sai số nhỏ hơn)
            if "mae" in key:
        ...
        lines.append("\n  [KẾT LUẬN]")
        if f1_delta > 0.01:
            lines.append("  ✅ Phương pháp cải tiến mang lại hiệu quả vượt trội")
            lines.append(f"     F1-macro tăng {f1_delta:+.4f} → Khuyến nghị áp dụng vào pipeline báo cáo.")
        elif f1_delta > 0:
            lines.append("  ✅ Hiệu suất tăng nhẹ hoặc tương đương ổn định.")
        else:
            lines.append("  ⚠️  Hiệu suất không tăng hoặc suy giảm nhẹ — cần tinh chỉnh hyperparameters.")
...
def rerun_inference(config_path: str) -> None:
    """Chạy lại inference trên test set với checkpoint của best model."""
    import yaml
    ...
    # Khởi tạo DataLoader tương ứng theo chế độ
    if binocular:
        from src.binocular_dataset import get_binocular_dataloaders
    ...
    model = build_model(
        ...
        pretrained      = False,       # Load từ checkpoint — không tải online
    ...
    if str(focal_alpha_val).lower().strip() == "auto":
        ...
        if binocular:
            from src.binocular_dataset import BinocularDataset
            from src.transforms import get_transforms
            train_ds_tmp = BinocularDataset(
                csv_path   = splits_dir / "train.csv",
                img_dir    = img_dir,
                transforms = get_transforms(mode="val", img_size=tr_cfg["img_size"]),
                img_size   = tr_cfg["img_size"],
            )
            labels_list = [p["label"] for p in train_ds_tmp.patients]
        else:
            import pandas as pd
            train_csv = splits_dir / "train.csv"
            df_train = pd.read_csv(train_csv)
            labels_list = (df_train['N'] == 0).astype(int).tolist()
        n_pos = sum(labels_list)  # Bệnh lý (Pathological, y=1)
        n_neg = len(labels_list) - n_pos  # Bình thường (Normal, y=0)
        focal_alpha_val = n_neg / len(labels_list)
        print(f"[Focal Loss] Rerun: Tự động cấu hình alpha = {focal_alpha_val:.4f}")
    else:
    ...
    # Nạp hàm chạy epoch từ file train.py đã sửa đổi
    sys.path.insert(0, str(project_root))
    from train import run_epoch

    # Tìm ngưỡng động validation trước khi test nếu ở chế độ test kết quả tối ưu
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exps", nargs="+",
        default=[
            "results/exp_1_cnn_binary_raw",
            "results/exp_2_cnn_binary_enhanced",
            "results/exp_3_cnn_binary_enhanced_aug",
            "results/exp_4_swin_binary_raw",
            "results/exp_5_swin_binary_enhanced",
            "results/exp_6_swin_binary_enhanced_aug",
        ],
